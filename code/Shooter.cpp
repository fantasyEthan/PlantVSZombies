...
Shooter::Shooter(int  as, int  ms, int  po_x, int  po_y, int hp, int p, int b):Plant(as, ms, po_x, po_y, hp, p,"Repeater"), bulletType(b) {
}
...
void Shooter::update()
{
	...
	if(!moveStatus && animationStatus){
		...
		if (hasZombie && (!workCounter) && (!has_stopped)) {
			switch (bulletType) {
			case 1:
			...
			{
				bullet.push_back(new PeaBullet(position_x, position_y + 5));
				bullet.push_back(new PeaBullet(position_x + 30, position_y + 5));
				break;
			}
			}
			workCounter = 8;
